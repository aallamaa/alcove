...
static void push(node *L, node *c) {
  if (L->n == L->cap) {
    ...
    L->kid = xrealloc(L->kid, L->cap * sizeof *L->kid);
  }
  L->kid[L->n++] = c;
}
...
static node *read_one(lex *r) {
  ...
  if (c == '#' && r->i + 1 < r->n) {
    ...
    if (d == '\\' && r->i + 2 < r->n) { /* #\X — keep whole UTF-8 codepoint */
      ...
      if (r->i + 2 + clen > r->n)
        clen = r->n - (r->i + 2);
      node *a = mk_atom(r->s + r->i, 2 + clen);
      r->i += 2 + clen;
      return a;
    }
    if (d == 'b' && r->i + 2 < r->n && r->s[r->i + 2] == '"') {
      size_t start = r->i;
      r->i += 2; /* keep #b, then the string verbatim */
      r->i++;    /* opening quote */
      while (r->i < r->n) {
        if (r->s[r->i] == '\\') {
          r->i += 2;
  ...
  if (c == '\'' || c == '`' || c == ',') {
    ...
    q->call = 0;
    q->tok =
        xstrndup(r->s + start, (c == ',' && r->s[start + 1] == '@') ? 2 : 1);
    push(q, read_one(r));
    return q;
  }
  /* bare token up to a delimiter */
  size_t start = r->i;
  while (r->i < r->n && !is_delim(r->s[r->i]))
    r->i++;
  if (r->i == start)
    r->i++; /* lone delimiter we don't special-case */
  return mk_atom(r->s + start, r->i - start);
}
...
static void read_forms(lex *r, char term, node *out) {
  ...
  for (;;) {
    while (r->i < r->n &&
           (is_ws(r->s[r->i]) || (sep_comma && r->s[r->i] == ',')))
      r->i++;
    if (r->i >= r->n)
      return;
    char c = r->s[r->i];
    ...
    /* name/result glued to '(' : a CALL `f(args)`. Chains only when the head is
       a SYMBOL atom (an identifier) or an already-glued call result. NOT a
       string literal (`"s"(x)` is two forms — a string then a call), and NOT a
       bare paren-list (`(a)(b)` is two forms — the Alcove packing — while a
       true chain `name(a)(b)` has f->call set). Keeps glued runs like
       `"msg"(get …)` and `(vec-set! v 0 x)(vec-set! v 1 y)` correctly
       separated. */
  ...
  for (size_t i = 0; i < n; i++) {
    ...
    if (c == '(' || c == '[' || c == '{')
      ...
      depth--;
             ...
             (body[i + 1] == ' ' || body[i + 1] == '\t'))
      return (
          int)i; /* `head: body` — not a :keyword (those are space/<letter>) */
  }
  return -1;
}
...
static void scan_line(const char *line, buf *out, int *depth, int *in_str,
                      int *cpos) {
  ...
  for (; i < n; i++) {
    ...
    if (c == '#') {
      ...
      if (d == '\\') { /* #\X — emit # \ and the whole codepoint */
        ...
        i += 2;
        if (i < n) {
          unsigned char l = (unsigned char)line[i];
...
static node *line_node(const char *text) {
  ...
  node *forms = mk_list('(');
  forms->open = 'L'; /* 'L' = bare line list */
  read_forms(&r, 0, forms);
  if (forms->n == 1) {
    node *only = forms->kid[0];
    forms->kid[0] = NULL;
    freenode(forms);
    return only;
  }
  /* ALCOVE INPUT ONLY: multiple top-level forms ALL parenthesized are separate
     statements packed on one line (`(a) (b) (c)`). In ADDER the same surface is
     a defn clause `(params) (body)` or an infix expr `(a) + (b)` — both kept
     grouped by adr.h's "many -> (f …)" rule — so this split is gated to .alc
     input. */
  if (g_alcove_mode && forms->n > 1) {
    int all_lists = 1;
    for (int k = 0; k < forms->n; k++)
      if (!forms->kid[k]->is_list) {
        all_lists = 0;
        break;
      }
    if (all_lists) {
...
static node *parse(const char *src) {
  enum { MAXD = 256 };
  int ind_stack[MAXD];
  node *node_stack[MAXD];
  int sp = 0;
  node *roots = mk_list('R');
  buf pend;
  buf_init(&pend); /* pending leading-comment block */
  int pend_blank = 0;
  ...
  while (i <= slen) {
    /* Read one LOGICAL form: a physical line, plus any continuation lines while
       ...
       strips comments and tracks (depth,in_str) across the join, so a
       multi-line s-expr (an .alc file) reads as a single form. The first
       physical line's comment is kept; continuation-line comments are dropped
       (rare in practice). */
    buf codeb;
    buf_init(&codeb);
    ...
    while (i <= slen) {
      ...
      if (lc >= 0 &&
          comment == NULL) { /* keep first line's comment, as Adder `#` */
        const char *cm = raw + lc;
        int s = 0;
        while (cm[s] == ';' || cm[s] == '#')
          s++; /* skip the marker run */
        while (cm[s] == ' ' || cm[s] == '\t')
          s++; /* and one gap */
        buf cb;
        buf_init(&cb);
      ...
      if (i > slen)
        break; /* unterminated form at EOF: take what we have */
    }
    ...
    {
      int k = 0;
      while (cstart[k] == ' ' || cstart[k] == '\t')
        k++;
      cstart += k;
    }
    char *body = xstrndup(cstart, strlen(cstart));
    for (size_t k = strlen(body); k && is_ws(body[k - 1]);)
      body[--k] = 0;
    (void)any;
    ...
    if (icolon >= 0) {
      ...
      if (block) {
        nd->block = 1;
        nd->hdr_n = nd->n;
      } /* indented body appends later */
      free(b2);
    }
    /* attach trivia */
    if (pend.len) {
      nd->lead = xstrndup(pend.p, pend.len);
      pend.len = 0;
      pend.p[0] = 0;
    }
    if (comment)
      nd->trail = xstrndup(comment, strlen(comment));
    nd->blank = pend_blank;
    pend_blank = 0;
    ...
    if (nd->open ==
        'M') { /* multiple statements packed on one line: split them */
      for (int k = 0; k < nd->n; k++) {
        node *child = nd->kid[k];
        if (k == 0) {
          child->lead = nd->lead;
          nd->lead = NULL;
          child->blank = nd->blank;
        }
        if (k == nd->n - 1) {
          child->trail = nd->trail;
          nd->trail = NULL;
        }
        push(parent_list, child);
      }
      nd->n = 0;
      freenode(nd); /* shell only — children re-homed */
      free(code);
      free(comment);
      free(body);
      continue;
  ...
  if (pend.len) {
    node *c = mk_list('C');
    c->lead = xstrndup(pend.p, pend.len);
    push(roots, c);
  }
  free(pend.p);
  return roots;
...
static int is_fn_head(const char *s) {
  return s && (!strcmp(s, "fn") || !strcmp(s, "lambda"));
}
...
static const char *head_tok(const node *x) {
  if (x->is_list && x->n > 0 && !x->kid[0]->is_list)
    return x->kid[0]->tok;
  return NULL;
}
...
/* Operator infix `(n < 0)` / `(n is 0)` / `(a + b)` reads better than prefix
   and computes the same VALUES, but it is NOT free: an unhinted-param
   comparison
...
static int is_infix_op_form(const node *x) {
  return !g_quoted && x->is_list && !x->call &&
         (x->open == '(' || x->open == 'L') && x->n == 3 &&
...
static void emit_inline(const node *x, buf *o) {
  ...
  if (x->open ==
      '\'') { /* quote/quasiquote/unquote — its body is literal data */
    int save = g_quoted;
    g_quoted = 1;
    buf_puts(o, x->tok);
    emit_inline(x->kid[0], o);
    g_quoted = save;
    return;
  }
  if (is_infix_op_form(x)) { /* (op a b) -> (a op b) */
    buf_putc(o, '(');
    emit_inline(x->kid[1], o);
    buf_putc(o, ' ');
    buf_puts(o, x->kid[0]->tok);
    buf_putc(o, ' ');
    emit_inline(x->kid[2], o);
    buf_putc(o, ')');
    return;
  }
  if (x->comp) {
...
static void put_indent(buf *o, int col) {
  for (int k = 0; k < col; k++)
    buf_putc(o, ' ');
}
...
static int emit_block_header(const node *x, buf *hdr, int hn_in) {
  ...
  if (x->call) {
    emit_inline(x->kid[0], hdr);
    buf_putc(hdr, '(');
    emit_seq(x->kid, 1, hn, hdr);
    buf_putc(hdr, ')');
    return hn;
  }
...
static int header_arity(const char *h) {
  ...
  struct {
    const char *n;
    int a;
  } t[] = {{"def", 3},   {"defn", 3}, {"defc", 3},   {"defmacro", 3},
           {"macro", 3}, {"mac", 3},  {"fn", 2},     {"lambda", 2},
           {"with", 2},  {"each", 2}, {"let", 3},    {"let*", 3},
           {"if", 2},    {"when", 2}, {"unless", 2}, {"while", 2},
           {"for", 4},   {"case", 2}, {"do", 1},     {0, 0}};
  for (int k = 0; t[k].n; k++)
    if (!strcmp(h, t[k].n))
      return t[k].a;
  return -1;
...
static int header_hn(const node *x) {
  if (x->block)
    return x->hdr_n > 0 ? x->hdr_n : x->n;
  if (!x->is_list || x->open != '(' || x->call)
    return 0;
...
/* Should this header form BREAK its body across indented lines, INDEPENDENT of
   width? Yes for the if-family (elif/else need an open block), >1 body form, or
   a body form that is itself a header (nested if/for/...). The width test is
   done separately by emit_form on the ACTUAL `header: body` candidate, so the
   two passes agree (avoids inline_width(x) — the paren form — disagreeing with
   the emitted line and causing a format/idempotency flip-flop). */
static int wants_break(const node *x, int hn) {
  if (is_if_family(head_tok(x)))
    return 1;
  if (x->n - hn > 1)
    return 1;
  for (int k = hn; k < x->n; k++)
    if (x->kid[k]->is_list && header_hn(x->kid[k]))
      return 1;
  return 0;
}
...
static void emit_stmt_line(const node *x, buf *o) {
  ...
  if (is_infix_op_form(
          x)) { /* (op a b) -> a op b (no outer parens at stmt level) */
    emit_inline(x->kid[1], o);
    buf_putc(o, ' ');
    buf_puts(o, x->kid[0]->tok);
    buf_putc(o, ' ');
...
static void emit_form(const node *x, int col, buf *o) {
  ...
  if (!wants_break(x, bodyfrom) && !x->kid[bodyfrom]->lead &&
      !x->kid[bodyfrom]->trail) {
    /* Try inline `header: body`. Measure the ACTUAL candidate (header + ": " +
       statement-style body) so the inline/break decision is stable across
       passes and matches what we emit. Fits in WIDTH -> inline; else fall
       through. */
    buf cand;
    buf_init(&cand);
    buf_puts(&cand, hdr.p);
...
static int adfmt_usage(FILE *o) {
  fputs("usage: adder fmt [options] [files...]\n"
        "  (no files, or \"-\")   read stdin, write formatted Adder to stdout\n"
        "  files...              read each file\n"
        "options:\n"
        "  -w, --write           rewrite each file in place (else print to "
        "stdout)\n"
        "      --check           exit non-zero if any file isn't already "
        "formatted\n"
        "                        (prints the unformatted paths; makes no "
        "changes)\n"
        "      --no-infix        keep prefix `(< n 0)` (default emits infix "
        "`(n < 0)`,\n"
        "                        which reads better; only cost is an unhinted "
        "numeric\n"
        "                        hot loop won't JIT-fuse — add an :int hint to "
        "recover)\n"
        "  -h, --help            this help\n",
        o);
  return 2;
...
int adfmt_cli_main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; i++) {
    const char *a = argv[i];
    if (!strcmp(a, "-w") || !strcmp(a, "--write"))
      write_inplace = 1;
    else if (!strcmp(a, "--check"))
      check = 1;
    else if (!strcmp(a, "--infix"))
      g_infix_ops = 1;
    else if (!strcmp(a, "--no-infix"))
      g_infix_ops = 0;
    else if (!strcmp(a, "--alcove"))
      force_alcove = 1;
  ...
  if (nfiles == 0 || (nfiles == 1 && !strcmp(files[0], "-"))) {
    if (write_inplace || check) {
      fputs("adfmt: --write/--check need file arguments\n", stderr);
      return 2;
    }
