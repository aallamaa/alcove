...
if [ -f test.alc ] && [ -x ./alcove ]; then
  ...
  # Default mode (prefix preserved) must be byte-identical in test RESULTS — every
  # assert, including the jit-introspection ones, survives. This is the contract:
  # `adder fmt FILE` changes layout only, never compiled/JIT behavior.
  if [ -n "$o" ] && [ "$o" = "$c" ]; then echo "  OK — $c (identical to alcove)";
  else echo "  MISMATCH: alcove='$o' vs adder-formatted='$c'"; fail=1; fi

  # OPT-IN infix mode (--infix) is correctness-preserving for VALUES, but may
  # change jit-introspection asserts (an unhinted-param comparison can lose JIT
  # fusion). Require: no assert reporting a WRONG VALUE — only jit-meta may differ.
  echo "== --infix is value-correct (only jit-meta asserts may differ) =="
  "$ADDER" fmt --infix test.alc > "$D/ti.adr" 2>/dev/null
